...
static long closed_list_cmp(Node *node1, Node *node2)
{
  ...
  return (long) state_cmp(node1->state, node2->state);
}
...
{ 
  ...
  node->id = (opt.random ? cpt_rand() : gdsl_rbtree_get_size(closed_list->list));
...
static void lists_insert_foreign(Node *node, int dest)
{
  ...
  node->id = (opt.random ? cpt_rand() : gdsl_rbtree_get_size(closed_lists[dest].list));
...
int yahsp_main(ulong nodes_limit_global)
{
  ...
  FOR(node_limit, node_limits) {
    ...
    //cpt_trace(normal, "#threads : %d -- already evaluated nodes : %lu\n", num_threads, stats.evaluated_nodes);
#ifdef YAHSP_MT
    // long-term forgotten...
    int i;
    for(i = 0; i < opt.yahsp_teams; i++) {
