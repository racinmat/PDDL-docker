...
#define vector_sort(name, cmp_func) NEST(long _i, _j; bool _loop = true; while (_loop) { _loop = false; for(_i = 0; _i < (long) name##_nb - 1; _i++) for(_j = _i + 1; _j < (long) name##_nb; _j++) if (cmp_func(name[_j], name[_i]) < 0) { _loop = true; exchange(name[_i], name[_j]); } } )
