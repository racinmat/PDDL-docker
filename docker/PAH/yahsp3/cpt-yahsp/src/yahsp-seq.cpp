...
static long closed_list_cmp(Node *node1, Node *node2)
{
  ...
  return state_cmp(node1->state, node2->state);
}
...
static Node *closed_list_insert(Node *node)
{
  ...
  node->id = (opt.random ? cpt_rand() : gdsl_rbtree_get_size(closed_list));
...
static Node *compute_node(Node *node)
{
  ...
  return compute_node(apply_relaxed_plan(node, best_makespan));
}
